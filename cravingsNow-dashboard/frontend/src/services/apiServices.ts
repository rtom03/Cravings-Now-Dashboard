...
// types/type.ts (add alongside existing types)
export interface UpdateProductInput {
  ...
  reactivateAt?: string | null; // ISO timestamp, or null to clear it
}
...
): Promise<Partial<UpdateProductInput>> => {
  try {
    return apiFetch(`/products/update/${id}`, {
      method: "PATCH",
      headers: {
        "Content-Type": "application/json",
      },
